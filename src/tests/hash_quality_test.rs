...
#[cfg(feature = "ahash_long_running_tests")]
...
#[cfg(feature = "ahash_long_running_tests")]
fn cast_u32_to_u8(s: &[u32]) -> &[u8] {
    let (ptr, len) = (s.as_ptr(), s.len());
    assert!(len * 4 < isize::MAX as usize);
    // SAFETY: ptr is valid, aligned, and length matches
    unsafe { core::slice::from_raw_parts(ptr as *const u8, len * 4) }
}
...
fn test_no_full_collisions<T: Hasher>(gen_hash: impl Fn() -> T) {
    ...
    for combination in combinations {
        let array = cast_u32_to_u8(&combination).to_vec();
...
#[cfg(test)]
mod fallback_tests {
    use crate::collections::hash_map::AHasher;
    use super::*;
